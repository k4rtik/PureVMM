...
u32int kmalloc_int(u32int sz, int align, u32int * phys)
{
	if (kheap != 0) {
		...
		if (phys != 0) {
			page_t *page =
			    get_page((u32int) addr, 0, kernel_directory);
...
static void expand(u32int new_size, heap_t * heap)
{
	...
	u32int i = old_size;
	while (i < new_size) {
		alloc_frame(get_page
			    (heap->start_address + i, 1, kernel_directory),
			    (heap->supervisor) ? 1 : 0,
			    (heap->readonly) ? 0 : 1);
		i += 0x1000 /* page size */ ;
	}
...
static u32int contract(u32int new_size, heap_t * heap)
{
	...
	while (new_size < i) {
		free_frame(get_page
			   (heap->start_address + i, 0, kernel_directory));
		i -= 0x1000;
	}
		    ...
		    u8int readonly)
{
	...
	heap->index =
	    place_ordered_array((void *)start, HEAP_INDEX_SIZE,
				&header_t_less_than);
...
void *alloc(u32int size, u8int page_align, heap_t * heap)
{
	...
	if (iterator == -1)	// If we didn't find a suitable hole
	{
		...
		while (iterator < heap->index.size) {
			u32int tmp =
			    (u32int) lookup_ordered_array(iterator,
							  &heap->index);
			if (tmp > value) {
				value = tmp;
				idx = iterator;
		...
		if (idx == -1) {
			...
			header->is_hole = 1;
			footer_t *footer =
			    (footer_t *) (old_end_address + header->size -
					  sizeof(footer_t));
			...
			header_t *header =
			    lookup_ordered_array(idx, &heap->index);
			header->size += new_length - old_length;
			// Rewrite the footer.
			footer_t *footer =
			    (footer_t *) ((u32int) header + header->size -
					  sizeof(footer_t));
			footer->header = header;
			footer->magic = HEAP_MAGIC;
		}
	...
	if (page_align && orig_hole_pos & 0xFFFFF000) {
		u32int new_location =
		    orig_hole_pos + 0x1000 /* page size */  -
		    (orig_hole_pos & 0xFFF) - sizeof(header_t);
		header_t *hole_header = (header_t *) orig_hole_pos;
		hole_header->size =
		    0x1000 /* page size */  - (orig_hole_pos & 0xFFF) -
		    sizeof(header_t);
	...
	if (orig_hole_size - new_size > 0) {
		header_t *hole_header =
		    (header_t *) (orig_hole_pos + sizeof(header_t) + size +
				  sizeof(footer_t));
		hole_header->magic = HEAP_MAGIC;
		hole_header->is_hole = 1;
		hole_header->size = orig_hole_size - new_size;
		footer_t *hole_footer =
		    (footer_t *) ((u32int) hole_header + orig_hole_size -
...
void free(void *p, heap_t * heap)
{
	...
	footer_t *test_footer =
	    (footer_t *) ((u32int) header - sizeof(footer_t));
	...
	header_t *test_header =
	    (header_t *) ((u32int) footer + sizeof(footer_t));
